...
the derivative with respect to \\(X\\) is
   ...
   \\(\quad L' = -\frac{1}{(1-X)^2}\cdot(-1) = \frac{1}{(1-X)^2} = L^2\\)
...
\\(X'\\) like \\(1\\) yields something like
   ...
   \\(\quad L' = L + X(L + X\cdot L')\\)

   \\(\quad = L + X\cdot L + X^2 \cdot L + X^3 \cdot L + \ldots\\)
