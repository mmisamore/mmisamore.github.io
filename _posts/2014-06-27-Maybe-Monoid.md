...
transformation \\(Just: Id \to Maybe\\). For any object \\(a\\) there is
therefore a morphism \\(Just\, a: a \to Maybe\, a\\). There is another endofunctor,
namely \\(\ast\\), sending every type to the unit type \\(()\\), and a
natural transformation \\(Nothing: \ast \to Maybe\\).
